...
class DataLoaderTrain(Dataset):
    def __init__(self, rgb_dir, args):
        ...
        self.inp_filenames = []
        self.args = args
        train_file_dirs = os.listdir(rgb_dir)
        for file_item in train_file_dirs:
            path = os.path.join(rgb_dir, file_item)
            print(path)
            with h5py.File(path, "r") as f:
                blur_image_name_list = sorted(
                    list(f["images"].keys())
                )  # sorted(list(f['blur_images'].keys()))
                sharp_image_name_list = sorted(list(f["sharp_images"].keys()))
                event_image_name_list = sorted(
                    list(f["voxels"].keys())
                )  # sorted(list(f['event_frames'].keys()))
                img_num = len(blur_image_name_list)
                for i in range(img_num):
                    blur_img = np.asarray(
                        np.float32(f["images"][blur_image_name_list[i]])/255.
                    )  # np.asarray(f['blur_images'][blur_image_name_list[i]]).transpose([2, 0, 1])
                    sharp_img = np.asarray(
                        np.float32(f["sharp_images"][sharp_image_name_list[i]])/255.
                    )
                    event_frame = np.asarray(
                        f["voxels"][event_image_name_list[i]]
                    )  # np.asarray(f['event_frames'][event_image_name_list[i]])
    ...
    def __getitem__(self, index):
        ...
        #print(0,data[0].shape)
        #print(0,data[0].max())
        #print(1,data[1].shape)
        #print(1,data[1].max())
        #print(2,data[2].shape)
        #print(2,data[2].max())
        #print(31,data[0].dtype)
        #print(32,data[1].dtype)
        #print(33,data[2].dtype)
        input_img, input_event, target = utils.image_proess(
            data[0], data[1], data[2], self.args.TRAINING.TRAIN_PS, self.args
        )
...
class DataLoaderVal(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     data[0], data[1], data[2], self.args.TRAINING.TRAIN_PS, self.args
        # )
        input_img = torch.from_numpy(data[0])
        input_event = torch.from_numpy(data[1])
        target = torch.from_numpy(data[2])
        data = (input_img, input_event, target)
        return data
